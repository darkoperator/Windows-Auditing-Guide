...
This document consolidates the XPath query collections previously maintained in two separate
files — the general Windows Event Collection query set and the Domain Controller-specific query
set. Overlapping categories (e.g. Authentication Events, Account Management, Object Access,
Process Creation, RDP, Firewall) have been merged into single queries with the union of their
event IDs, duplicates removed. Categories unique to one source (Certificate Services, WMI,
Windows Defender from the general set; Directory Service Events, Group Management, and the
command-line-aware Process Creation query from the Domain Controller set) are preserved.
  ...
  <Query Id="0" Path="Security">
    <Select Path="Security">
      *[System[(EventID=4720) or (EventID=4722) or (EventID=4723) or (EventID=4724) or
        (EventID=4725) or (EventID=4726) or (EventID=4738) or (EventID=4740) or
        (EventID=4767) or (EventID=4781)]]
    </Select>
  </Query>
</QueryList>
```
...
<QueryList>
  <Query Id="0" Path="Security">
    <Select Path="Security">
      *[System[(EventID=4728) or (EventID=4729) or (EventID=4732) or (EventID=4733) or
...
Requires "Include command line in process creation events" to be enabled (via Group Policy or
the registry) so that `CommandLine` data is populated on Event ID 4688.
```xml
<QueryList>
  <Query Id="0" Path="Security">
    <Select Path="Security">
...
<QueryList>
  <Query Id="0" Path="Directory Service">
    <Select Path="Directory Service">
      *[System[(EventID=4662) or (EventID=5136) or (EventID=5137) or (EventID=5138) or
        (EventID=5139) or (EventID=5141)]]
    </Select>
  </Query>
</QueryList>
```
...
### Critical System and Service Events
```xml
<QueryList>
  <Query Id="0" Path="System">
    <Select Path="System">
      *[System[(EventID=1102) or (EventID=6005) or (EventID=6006) or (EventID=7022) or
        (EventID=7023) or (EventID=7024) or (EventID=7026) or (EventID=7031) or
...
Certificate Services auditing spans 31 event IDs (4868-4898), which exceeds the 20-event-ID
limit per query, so the range is split across two queries.
...
## Notes
1. Each query is limited to 20 event IDs per Windows limitations. This constraint is why the
   Certificate Services range (31 IDs) above is split into two separate queries — apply the same
   pattern if you extend any category past 20 IDs.
...
8. The Process Creation with Command-Line Details query only returns command-line data if
   command-line process auditing is enabled; otherwise the `CommandLine` field is empty.
